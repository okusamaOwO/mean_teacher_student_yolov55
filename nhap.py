...
def train(hyp, opt, device, callbacks):
    ...
    if pretrained:
        ...
        student_model = Model(cfg or ckpt["model"].yaml, ch=3, nc=nc, anchors=hyp.get(
        ...
        LOGGER.info(
            # report
            f"Transferred {len(csd)}/{len(student_model.state_dict())} items from {weights}")
    else:
    ...
    if RANK == -1 and batch_size == -1:  # single-GPU only, estimate best batch size
        batch_size = check_train_batch_size(student_model, imgsz, amp)
    ...
    optimizer = smart_optimizer(
        student_model, opt.optimizer, hyp["lr0"], hyp["momentum"], hyp["weight_decay"])
    # mình đang muốn load state dict của thằng student xem là nó có những tham số gì
    # Scheduler
    if opt.cos_lr:
    ...
    if opt.sync_bn and cuda and RANK != -1:
        student_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            if not opt.noautoanchor:
                check_anchors(dataset, model=student_model,
                              # run AutoAnchor
    ...
    # student_model attributes
    # number of detection layers (to scale hyps)
    nl = de_parallel(student_model).model[-1].nl
    hyp["box"] *= 3 / nl  # scale to layers
    hyp["cls"] *= nc / 80 * 3 / nl  # scale to classes and layers
    hyp["obj"] *= (imgsz / 640) ** 2 * 3 / nl  # scale to image size and layers
    hyp["label_smoothing"] = opt.label_smoothing
    student_model.nc = nc  # attach number of classes to model
    student_model.hyp = hyp  # attach hyperparameters to model
    student_model.class_weights = labels_to_class_weights(
    ...
    for epoch in range(start_epoch, epochs):
        if epoch + opt.trained_epochs < 20:
            unsupervised_loss_weight = 0.3 *  (1 - math.cos((epoch + opt.trained_epochs) / 20 * math.pi / 2))
        else:
        ...
        if opt.image_weights:
            cw = student_model.class_weights.cpu().numpy() * (1 - maps) ** 2 / \
                nc  # class weights
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                # process nms_pred
                print(
                    f"supervised loss: {loss.item():.4f}, unsupervised loss: {unsupervised_loss.item():.4f}, total loss: {total_loss.item():.4f}")
        ...
        if RANK in {-1, 0}:
            ...
            ema.update_attr(student_model, include=[
