        ...
        for i, ind in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                with torch.no_grad():
                    ...
                    pred_teacher_foggy_nms = non_max_suppression(
                ...
                per_batch_size, channels, height, width = tar_imgs.shape
                pred_labels_out_batch = []
                for img_id in range(per_batch_size):
                    labels_num = pred_teacher_foggy_nms[img_id].shape[0]
                    if labels_num:
                        labels_list = torch.cat((pred_teacher_foggy_nms[img_id][:, 5].unsqueeze(-1),
                            pred_teacher_foggy_nms[img_id][:, 0:4]), dim=1)  # remove predicted conf, new format [cls x y x y]
                ...
                if len(pred_labels_out_batch) != 0:
                    ...
                    pred_labels = torch.from_numpy(
                        np.array([[0, 0, 0.5, 0.5, random.uniform(0.2, 0.8), random.uniform(0.2, 0.8)]])).to(device)
                print(pred_labels[0])
                pred_student_foggy = model_student(tar_imgs)
                loss_distillation, loss_items_distillation = compute_loss(
                    pred_student_foggy, pred_labels)
