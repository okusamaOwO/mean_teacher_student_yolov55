...
def train(hyp, opt, device, callbacks):
    ...
    if pretrained:
        ...
        csd = intersect_dicts(csd, student_model.state_dict(), exclude=exclude)  # intersect
        student_model.load_state_dict(csd, strict=False)  # load
        LOGGER.info(f"Transferred {len(csd)}/{len(student_model.state_dict())} items from {weights}")  # report
    else:
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            if not opt.noautoanchor:
                check_anchors(dataset, model=student_model, thr=hyp["anchor_t"], imgsz=imgsz)  # run AutoAnchor
    ...
    nl = de_parallel(student_model).model[-1].nl  # number of detection layers (to scale hyps)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni - last_opt_step >= accumulate:
                ...
                torch.nn.utils.clip_grad_norm_(student_model.parameters(), max_norm=10.0)  # clip gradients
